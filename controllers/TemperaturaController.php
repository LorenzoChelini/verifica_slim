...
class TemperaturaController {
    public function getRilevatori(Request $request, Response $response, $args) {
        ...
        $rilevatoriTemperatura = array_filter($impianto->rilevatori, function ($rilevatore) {
            return $rilevatore instanceof RilevatoreDiTemperatura;
        });
        return $response->withJson($rilevatoriTemperatura);
    }
